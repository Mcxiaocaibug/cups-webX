...
│   │   ├── App.vue            # 根组件
│   │   └── main.js            # 入口文件
│   ├── go.mod                 # Go 子模块边界，避免主模块扫描 node_modules
│   ├── package.json           # 前端依赖
...
2. **部署向导** (`SetupView.vue`)
   - 首次部署时配置 CUPS 地址和管理员密码
   - 测试 CUPS 连通性并完成初始化

3. **打印页面** (`PrintView.vue`)
   - 文件上传和打印控制
   - 页数估算、打印预设、历史筛选、账户密码修改
   - 历史任务重打与活动任务取消
   - 打印状态自动同步为排队中 / 处理中 / 已打印 / 失败 / 已取消
   - 失败原因和状态说明展示
   - 问题聚类筛选（失败原因、排队超时、处理中超时）
   - 管理员重置密码后的强制改密流程

4. **管理页面** (`AdminView.vue`)
   - 用户管理和系统设置
   - 记录筛选、CSV 导出、重打和取消
   - 文件名下方展示打印参数摘要
   - 有活动任务时自动刷新记录状态
...
# make build 优先用 Bun，失败时会自动回退到 npm ci
...
# 只做 Go 侧快速编译时，可直接运行 go build ./cmd/server
# 此时会内嵌 fallback 页面，适合本地调试和测试。
```

补充说明：
- Vite 构建产物输出到 `internal/webui/dist`
- `frontend/go.mod` 仅用于隔离主模块测试扫描，不参与应用运行
...
# 或直接运行 Web 镜像
docker run -d --name cups-web -p 1180:8080 -v cups-web-data:/data -v cups-web-uploads:/uploads cups-web:latest
...
| `CUPS_HOST` | `localhost:631` | 首次部署向导默认展示的 CUPS 地址；保存后会写入数据库设置 |
| `SESSION_HASH_KEY` | 自动生成 | Session 哈希密钥；生产环境建议显式指定以保持会话稳定 |
| `SESSION_BLOCK_KEY` | 自动生成 | Session 块密钥；生产环境建议显式指定以保持会话稳定 |
...
`users` 表额外包含 `must_change_password` 字段，用于强制初始密码和管理员重置密码后的首次改密流程。

`settings` 表额外保存 `cups_host` 和 `setup_complete`，用于网页端部署向导和运行期配置读取。
...
- `POST /api/me/password` - 当前用户修改密码

### 打印接口
- `GET /api/printers` - 获取打印机列表
- `POST /api/print` - 提交打印任务
- `POST /api/estimate` - 估算文件页数
- `GET /api/print-records` - 获取打印记录
- `POST /api/print-records/{id}/retry` - 按原参数重新提交打印
- `POST /api/print-records/{id}/cancel` - 取消活动中的打印任务
