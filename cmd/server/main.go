...
func main() {
	...
	if err := bootstrapAppState(context.Background()); err != nil {
		log.Fatal("failed to bootstrap app state: ", err)
	}
	...
	api.HandleFunc("/setup/test-cups", setupTestCUPSHandler).Methods("POST")
	api.HandleFunc("/setup/bootstrap", setupBootstrapHandler).Methods("POST")
	...
	protected.HandleFunc("/me/password", changeMyPasswordHandler).Methods("POST")

	ready := protected.PathPrefix("").Subrouter()
	ready.Use(requirePasswordChangeCleared)
	ready.HandleFunc("/printers", func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")

		cupsHost, err := currentCUPSHost(r.Context())
		if err != nil {
			http.Error(w, "failed to load printer host: "+err.Error(), http.StatusInternalServerError)
			return
		}
	...
	ready.HandleFunc("/print", printHandler).Methods("POST")
	ready.HandleFunc("/convert", convertHandler).Methods("POST")
	ready.HandleFunc("/estimate", estimateHandler).Methods("POST")
	ready.HandleFunc("/print-records", printRecordsHandler).Methods("GET")
	ready.HandleFunc("/print-records/{id:[0-9]+}/file", printRecordFileHandler).Methods("GET")
	ready.HandleFunc("/print-records/{id:[0-9]+}/retry", retryPrintRecordHandler).Methods("POST")
	ready.HandleFunc("/print-records/{id:[0-9]+}/cancel", cancelPrintRecordHandler).Methods("POST")
	ready.HandleFunc("/printer-info", printerInfoHandler).Methods("GET")

	admin := api.PathPrefix("/admin").Subrouter()
	admin.Use(middleware.RequireSession)
	admin.Use(requirePasswordChangeCleared)
	...
	serverFS := server.NewEmbeddedServer(webui.FS)
