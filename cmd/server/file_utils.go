...
func resolveStoredPrintPath(storedRel string, baseDir string) (string, string, error) {
	if storedRel == "" {
		return "", "", errors.New("missing stored path")
	}

	convertedRel := convertedRelPath(storedRel)
	if convertedRel != "" {
		convertedAbs := filepath.Join(baseDir, filepath.FromSlash(convertedRel))
		if _, err := os.Stat(convertedAbs); err == nil {
			return convertedRel, convertedAbs, nil
		}
	}

	absPath := filepath.Join(baseDir, filepath.FromSlash(storedRel))
	if _, err := os.Stat(absPath); err != nil {
		return "", "", err
	}
	return storedRel, absPath, nil
}

func detectStoredFileMIME(path string) (string, error) {
	f, err := os.Open(path)
	if err != nil {
		return "", err
	}
