...
func CountAdmins(ctx context.Context, tx *sql.Tx) (int, error) {
	var count int
	if err := tx.QueryRowContext(ctx, "SELECT COUNT(1) FROM users WHERE role = ?", RoleAdmin).Scan(&count); err != nil {
		return 0, err
	}
	return count, nil
}
...
func GetUserByUsername(ctx context.Context, tx *sql.Tx, username string) (User, error) {
	row := tx.QueryRowContext(ctx, `SELECT
		id, username, password_hash, role, protected, must_change_password, contact_name, phone, email,
...
func CreateUser(ctx context.Context, tx *sql.Tx, input CreateUserInput) (User, error) {
	...
	res, err := tx.ExecContext(ctx, `INSERT INTO users (
		username, password_hash, role, protected, must_change_password, contact_name, phone, email,
		created_at, updated_at
	) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		input.Username, input.PasswordHash, input.Role, input.Protected, input.MustChangePassword, input.ContactName, input.Phone, input.Email,
		now, now,
	)
	if err != nil {
		return User{}, err
...
func UpdateUser(ctx context.Context, tx *sql.Tx, input UpdateUserInput) (User, error) {
	...
	if input.PasswordHash != nil {
		...
		if _, err := tx.ExecContext(ctx, `UPDATE users SET
			username = ?, password_hash = ?, must_change_password = ?, role = ?, contact_name = ?, phone = ?, email = ?,
			updated_at = ?
			WHERE id = ?`,
			input.Username, *input.PasswordHash, mustChangePassword, input.Role, input.ContactName, input.Phone, input.Email,
			now, input.ID,
		); err != nil {
			return User{}, err
		}
...
func UpdateUserPassword(ctx context.Context, tx *sql.Tx, id int64, passwordHash string) error {
	res, err := tx.ExecContext(ctx, `UPDATE users SET
		password_hash = ?, must_change_password = 0, updated_at = ?
		WHERE id = ?`,
		passwordHash, nowUTC(), id,
	)
	if err != nil {
		return err
	}
...
func scanUser(s scanner) (User, error) {
	var user User
	err := s.Scan(
		&user.ID, &user.Username, &user.PasswordHash, &user.Role, &user.Protected, &user.MustChangePassword, &user.ContactName, &user.Phone, &user.Email,
